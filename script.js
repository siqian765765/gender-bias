...
const colors = {
  ...
  ink: "#1d1d1f",
  moss: "#26364f",
  muted: "rgba(66,66,69,.72)"
};

const bPaths = {
  sankey: "B%E9%83%A8%E5%88%86/B-%CD%BC1/data/peer_review_sankey.json",
  dot: "B%E9%83%A8%E5%88%86/B-%CD%BC2/data/acceptance_by_author_group_area.json",
  recommendation: "B%E9%83%A8%E5%88%86/B-%CD%BC2/data/recommendation_by_author_group_area.csv"
};

const rPath = "R%E9%83%A8%E5%88%86/data/desk_rejection_by_author_group_area.csv";
...
const bDotColors = {
  ...
  "Solo woman": "#C06C84",
  "All men team": "#4F6F9F",
  "All women team": "#B07A2A",
  "Cross collaboration": "#2F9C95"
};
...
const recColors = {
  "Accept": "#2F9C95",
  "Minor revisions": "#4F8CC9",
  "Major revisions": "#B07A2A"
};
...
let checkpointView = "all";
let simulatorState = {
  representation: 0.08,
  deskGap: 0.55,
  recommendationGap: 0.35,
  reviewBoost: 0.04
};

const cPaths = {
  prediction: "C%E9%83%A8%E5%88%86/figure5/data/figure5_prediction_long.csv",
  validation: "C%E9%83%A8%E5%88%86/figure5/data/figure5_bntest_grouped.csv"
};
const cDisciplines = [
...
function setupMethodAccordions() {
  document.querySelectorAll(".method-card").forEach((card) => {
    ...
    toggle.addEventListener("click", () => {
      ...
      document.querySelectorAll(".method-card.open").forEach((openCard) => {
        if (openCard === card) return;
        openCard.classList.remove("open");
        openCard.querySelector(".method-toggle")?.setAttribute("aria-expanded", "false");
      });
...
function drawRPerson(group, type, x, y, size) {
  return group
    .append("image")
    .attr("class", "r-person")
    .attr("href", type === "woman" ? "R%E9%83%A8%E5%88%86/women-restroom.svg" : "R%E9%83%A8%E5%88%86/men-restroom.svg")
    .attr("x", x)
    .attr("y", y)
    .attr("width", size)
    .attr("height", size)
...
function renderBSankey(data) {
  ...
  const height = 720;
  const margin = { top: 66, right: 178, bottom: 36, left: 120 };
  ...
  const finalDecisionLeft = d3.min(
    graph.nodes.filter((d) => getBStage(d.name) === "Final Decision"),
    (d) => d.x0
  );
  const linkClipId = `b-sankey-link-clip-${Math.random().toString(36).slice(2)}`;
  ...
  defs.append("clipPath")
    ...
    .attr("y", margin.top - 2)
    .attr("width", Math.max(1, finalDecisionLeft - margin.left + 2))
...
function renderRecommendationChart(data) {
  ...
  const color = d3.scaleSequential()
    ...
    .interpolator(d3.interpolateRgbBasis(["#efe3d5", "#d4b8a9", "#9fb1bf", "#5f879b", "#2f657d"]));
  ...
  cells.append("rect")
    ...
    .on("mousemove", function (event, d) {
      ...
      showBTooltip(event, `
        ...
        <span>${d.area} 路 ${rGroupDisplay.get(d.author_group) || d.author_group}</span><br/>
        Accept: ${d3.format(".1%")(d.accept_share)} 路 Minor revisions: ${d3.format(".1%")(d.minor_share)}<br/>
...
function normalizeAreaName(value) {
  return String(value || "")
    .replace(/\s+/g, " ")
    .replace("Soc. sciences & humanities", "Social sciences & humanities")
    .trim();
}

function areaKey(value) {
  return normalizeAreaName(value).toLowerCase();
}
...
function pct(value, digits = 1) {
  return d3.format(`.${digits}%`)(value || 0);
}

function diffColor(value) {
  if (value > 0.015) return "#2F7DD1";
  if (value < -0.015) return "#B85C5C";
  return "#8F98A3";
}
...
function buildRecommendationRows(data) {
  const grouped = d3.group(data || [], (d) => areaKey(d.area), (d) => d.author_group);
  const rows = [];

  cDisciplines.forEach((area) => {
    Array.from(rGroupIconMap.keys()).forEach((group) => {
      const values = grouped.get(areaKey(area))?.get(group) || [];
      const byRec = new Map(values.map((d) => [d.recommendation, d]));
      const accept = byRec.get("Accept") || { count: 0, share: 0, total: values[0]?.total || 0 };
      const minor = byRec.get("Minor revisions") || { count: 0, share: 0, total: values[0]?.total || 0 };
      const total = values[0]?.total || accept.total || minor.total || 0;
      rows.push({
        area,
        author_group: group,
        total,
        positive_count: accept.count + minor.count,
        positive_share: accept.share + minor.share
      });
    });
  });
...
function buildAcceptanceRows(data) {
  return (data?.records || []).map((d) => ({
    ...d,
    area: normalizeAreaName(d.area)
  }));
}

function buildCheckpointRows() {
  if (!rDeskRejectionCache || !bRecommendationCache || !bDotCache) return [];

  const groups = Array.from(rGroupIconMap.keys());
  const recRows = buildRecommendationRows(bRecommendationCache);
  const accRows = buildAcceptanceRows(bDotCache);
  const recByKey = new Map(recRows.map((d) => [`${areaKey(d.area)}|${d.author_group}`, d]));
  const accByKey = new Map(accRows.map((d) => [`${areaKey(d.area)}|${d.author_group}`, d]));
  const deskByKey = new Map(rDeskRejectionCache.map((d) => [`${areaKey(d.area)}|${d.author_group}`, d]));

  const recAreaAverage = new Map(
    d3.rollups(
      recRows,
      (rows) => d3.sum(rows, (d) => d.positive_count) / d3.sum(rows, (d) => d.total),
      (d) => areaKey(d.area)
    )
  );
  const accAreaAverage = new Map(
    d3.rollups(
      accRows,
      (rows) => d3.sum(rows, (d) => d.accepted) / d3.sum(rows, (d) => d.manuscripts),
      (d) => areaKey(d.area)
    )
  );
  ...
  cDisciplines.forEach((area) => {
    groups.forEach((group) => {
      const key = `${areaKey(area)}|${group}`;
      const desk = deskByKey.get(key);
      const rec = recByKey.get(key);
      const acc = accByKey.get(key);
      if (!desk || !rec || !acc) return;

      rows.push({
        area,
        author_group: group,
        author_group_label: rGroupDisplay.get(group) || group,
        manuscripts: desk.manuscripts,
        checkpoints: [
          {
            key: "desk",
            label: "Passed desk",
            value: 1 - desk.desk_rejection_rate,
            baseline: 1 - desk.area_desk_rejection_rate,
            diff: desk.area_desk_rejection_rate - desk.desk_rejection_rate
          },
          {
            key: "review",
            label: "Positive recs",
            value: rec.positive_share,
            baseline: recAreaAverage.get(areaKey(area)),
            diff: rec.positive_share - recAreaAverage.get(areaKey(area))
          },
          {
            key: "final",
            label: "Final accepted",
            value: acc.acceptance_rate,
            baseline: accAreaAverage.get(areaKey(area)),
            diff: acc.acceptance_rate - accAreaAverage.get(areaKey(area))
          }
        ]
      });
    });
...
function renderCheckpointControls() {
  const controls = d3.select("#checkpoint-controls");
  ...
  controls
    ...
    .join("button")
    .attr("type", "button")
    .classed("active", (d) => d.key === checkpointView)
    .text((d) => d.label)
    .on("click", (_, d) => {
      checkpointView = d.key;
      renderCheckpointMatrix();
    });
}

function renderCheckpointMatrix() {
  const container = d3.select("#checkpoint-chart");
  if (container.empty()) return;
  const rows = buildCheckpointRows();
  if (!rows.length) return;

  renderCheckpointControls();
  container.selectAll("*").remove();

  const areas = cDisciplines;
  const groups = Array.from(rGroupIconMap.keys());
  const width = Math.max(container.node().clientWidth, 1180);
  const margin = { top: 92, right: 46, bottom: 72, left: 238 };
  const rowHeight = 126;
  const height = margin.top + margin.bottom + rowHeight * areas.length;

  const x = d3.scaleBand().domain(groups).range([margin.left, width - margin.right]).paddingInner(0.08);
  const y = d3.scaleBand().domain(areas).range([margin.top, height - margin.bottom]).paddingInner(0.16);
  const nodeX = d3.scalePoint().domain(["desk", "review", "final"]).range([28, x.bandwidth() - 28]);

  const svg = container.append("svg").attr("width", "100%").attr("height", height).attr("viewBox", [0, 0, width, height]);

  svg.append("g")
    .attr("class", "checkpoint-columns")
    .selectAll("text")
    .data(groups)
    .join("text")
    .attr("x", (d) => x(d) + x.bandwidth() / 2)
  ...
  svg.append("g")
    .attr("class", "checkpoint-step-labels")
    .selectAll("text")
    .data(["Desk", "Review", "Final"])
    .join("text")
    .attr("x", (d, i) => margin.left + i * 66)
    .attr("y", margin.top - 16)
    .text((d) => d);

  const lanes = svg.append("g")
    .selectAll("g.checkpoint-lane")
    .data(areas)
    .join("g")
    .attr("class", "checkpoint-lane")
    .attr("transform", (d) => `translate(0,${y(d)})`);

  lanes.append("rect")
    .attr("class", "checkpoint-row-bg")
    .attr("x", 18)
    .attr("y", 0)
    .attr("width", width - 36)
    .attr("height", y.bandwidth())
  ...
  lanes.append("text")
    .attr("class", "checkpoint-area")
    .attr("x", 44)
    .attr("y", 38)
    .text((d) => cDisciplineLabels.get(d) || d);

  lanes.append("text")
    .attr("class", "checkpoint-note")
    .attr("x", 44)
    .attr("y", 64)
    .text("Relative to discipline average");

  const cells = svg.append("g")
    .selectAll("g.checkpoint-cell")
    .data(rows)
    .join("g")
    .attr("class", "checkpoint-cell")
    .attr("transform", (d) => `translate(${x(d.author_group)},${y(d.area)})`);

  cells.append("rect")
    .attr("class", "checkpoint-hit")
    .attr("width", x.bandwidth())
    .attr("height", y.bandwidth())
    .attr("rx", 12)
    .on("mousemove", (event, d) => {
      const lines = d.checkpoints.map((c) => `${c.label}: ${pct(c.value)} (${diffText(c.diff)})`).join("<br/>");
      showBTooltip(event, `
        <strong>${d.author_group_label}</strong>
        <span>${d.area}</span><br/>
        ${lines}<br/>
        Manuscripts: ${formatBNumber(d.manuscripts)}
      `);
    })
    .on("mouseleave", hideBTooltip);

  cells.append("path")
    .attr("class", "checkpoint-link")
    .attr("d", (d) => d3.line()
      .x((c) => nodeX(c.key))
      .y((c) => 48 - Math.max(-18, Math.min(18, c.diff * 260)))
      .curve(d3.curveMonotoneX)(d.checkpoints));

  const nodes = cells.append("g")
    .selectAll("g.checkpoint-node")
    .data((d) => d.checkpoints.map((c) => ({ ...c, parent: d })))
    .join("g")
    .attr("class", "checkpoint-node")
    .classed("is-muted", (d) => checkpointView !== "all" && checkpointView !== d.key)
    .attr("transform", (d) => `translate(${nodeX(d.key)},${48 - Math.max(-18, Math.min(18, d.diff * 260))})`);

  nodes.append("circle")
    .attr("r", (d) => 7 + Math.min(5, Math.abs(d.diff) * 70))
    .attr("fill", (d) => diffColor(d.diff));

  nodes.append("text")
    .attr("y", 28)
    .attr("text-anchor", "middle")
    .text((d) => diffText(d.diff));
}

function isWomenInvolved(group) {
  return group === "Solo woman" || group === "All women team" || group === "Cross collaboration";
}

function buildSimulatorBase() {
  if (!rDeskRejectionCache || !bRecommendationCache || !bDotCache) return null;

  const recRows = buildRecommendationRows(bRecommendationCache);
  const accRows = buildAcceptanceRows(bDotCache);
  const recByGroup = d3.rollup(
    recRows,
    (rows) => ({
      positive: d3.sum(rows, (d) => d.positive_count),
      total: d3.sum(rows, (d) => d.total)
    }),
    (d) => d.author_group
  );
  const accByGroup = d3.rollup(
    accRows,
    (rows) => ({
      accepted: d3.sum(rows, (d) => d.accepted),
      manuscripts: d3.sum(rows, (d) => d.manuscripts)
    }),
    (d) => d.author_group
  );
  const deskByGroup = d3.rollup(
    rDeskRejectionCache,
    (rows) => ({
      submitted: d3.sum(rows, (d) => d.manuscripts),
      deskRejected: d3.sum(rows, (d) => d.desk_rejected)
    }),
    (d) => d.author_group
  );

  const groups = Array.from(rGroupIconMap.keys()).map((group) => {
    const desk = deskByGroup.get(group) || { submitted: 0, deskRejected: 0 };
    const rec = recByGroup.get(group) || { positive: 0, total: 0 };
    const acc = accByGroup.get(group) || { accepted: 0, manuscripts: desk.submitted };
    const sent = Math.max(0, desk.submitted - desk.deskRejected);
    return {
      group,
      label: rGroupDisplay.get(group) || group,
      women: isWomenInvolved(group),
      submitted: desk.submitted,
      deskRejected: desk.deskRejected,
      sent,
      positive: rec.positive,
      accepted: acc.accepted,
      deskRate: desk.submitted ? desk.deskRejected / desk.submitted : 0,
      positiveRate: rec.total ? rec.positive / rec.total : 0,
      acceptanceRate: acc.manuscripts ? acc.accepted / acc.manuscripts : 0
    };
  });

  const totals = summarizeSimulatorGroups(groups);
  return {
    groups,
    overallDeskRate: totals.submitted ? totals.deskRejected / totals.submitted : 0,
    overallPositiveRate: totals.sent ? totals.positive / totals.sent : 0,
    overallAcceptanceRate: totals.submitted ? totals.accepted / totals.submitted : 0
  };
}

function summarizeSimulatorGroups(groups) {
  return {
    submitted: d3.sum(groups, (d) => d.submitted),
    deskRejected: d3.sum(groups, (d) => d.deskRejected || d.submitted - d.sent),
    sent: d3.sum(groups, (d) => d.sent),
    positive: d3.sum(groups, (d) => d.positive),
    accepted: d3.sum(groups, (d) => d.accepted),
    womenSubmitted: d3.sum(groups.filter((d) => d.women), (d) => d.submitted),
    womenAccepted: d3.sum(groups.filter((d) => d.women), (d) => d.accepted)
  };
}

function applySimulatorScenario(base) {
  const totals = summarizeSimulatorGroups(base.groups);
  const currentWomenShare = totals.womenSubmitted / totals.submitted;
  const targetWomenShare = Math.max(0.08, Math.min(0.92, currentWomenShare + simulatorState.representation));
  const targetWomen = totals.submitted * targetWomenShare;
  const targetMen = totals.submitted - targetWomen;
  const currentMen = totals.submitted - totals.womenSubmitted;
  const womenMultiplier = totals.womenSubmitted ? targetWomen / totals.womenSubmitted : 1;
  const menMultiplier = currentMen ? targetMen / currentMen : 1;

  return base.groups.map((d) => {
    const submitted = d.submitted * (d.women ? womenMultiplier : menMultiplier);
    const deskRate = clamp01(base.overallDeskRate + (d.deskRate - base.overallDeskRate) * (1 - simulatorState.deskGap));
    const sent = submitted * (1 - deskRate);
    const positiveRate = clamp01(
      base.overallPositiveRate +
      (d.positiveRate - base.overallPositiveRate) * (1 - simulatorState.recommendationGap) +
      (d.women ? simulatorState.reviewBoost : 0)
    );
    const positive = sent * positiveRate;
    const acceptanceRate = clamp01(
      base.overallAcceptanceRate +
      (d.acceptanceRate - base.overallAcceptanceRate) * (1 - simulatorState.recommendationGap * 0.25) +
      (d.women ? simulatorState.reviewBoost * 0.35 : 0)
    );
    const accepted = submitted * acceptanceRate;

    return {
      ...d,
      submitted,
      deskRejected: submitted * deskRate,
      sent,
      positive,
      accepted
    };
...
function renderSimulatorPresets() {
  const presets = [
    { key: "current", label: "Current", values: { representation: 0, deskGap: 0, recommendationGap: 0, reviewBoost: 0 } },
    { key: "balanced", label: "Balanced", values: { representation: 0.08, deskGap: 0.55, recommendationGap: 0.35, reviewBoost: 0.04 } },
    { key: "parity", label: "Parity", values: { representation: 0.16, deskGap: 0.9, recommendationGap: 0.75, reviewBoost: 0.06 } }
  ];
  const container = d3.select("#simulator-presets");
  if (container.empty()) return;
  ...
  container.selectAll("button")
    ...
    .text((d) => d.label)
    .on("click", (_, d) => {
      simulatorState = { ...d.values };
      renderFairnessSimulator();
    });
}

function renderSimulatorControls() {
  const controls = d3.select("#simulator-controls");
  if (controls.empty()) return;

  const sliders = [
    { key: "representation", label: "Women-involved submissions", min: -0.08, max: 0.20, step: 0.01, format: (v) => `${v >= 0 ? "+" : ""}${Math.round(v * 100)} pts` },
    { key: "deskGap", label: "Desk-screening gap reduction", min: 0, max: 1, step: 0.05, format: (v) => pct(v, 0) },
    { key: "recommendationGap", label: "Recommendation gap reduction", min: 0, max: 1, step: 0.05, format: (v) => pct(v, 0) },
    { key: "reviewBoost", label: "Reviewer diversity boost", min: 0, max: 0.08, step: 0.005, format: (v) => `+${Math.round(v * 100)} pts` }
  ];

  const rows = controls.selectAll("label").data(sliders, (d) => d.key).join("label");
  rows.html("");
  rows.append("span").text((d) => d.label);
  rows.append("output").text((d) => d.format(simulatorState[d.key]));
  rows.append("input")
    .attr("type", "range")
    .attr("min", (d) => d.min)
    .attr("max", (d) => d.max)
    .attr("step", (d) => d.step)
    .attr("value", (d) => simulatorState[d.key])
    .on("input", (event, d) => {
      simulatorState[d.key] = +event.currentTarget.value;
      renderFairnessSimulator();
    });
}

function renderSimulatorChart(base, scenarioGroups) {
  const container = d3.select("#simulator-chart");
  if (container.empty()) return;
  container.selectAll("*").remove();

  const baseline = base.groups;
  const baselineTotals = summarizeSimulatorGroups(baseline);
  const scenarioTotals = summarizeSimulatorGroups(scenarioGroups);
  const stages = [
    { key: "submitted", label: "Submitted" },
    { key: "sent", label: "Sent to review" },
    { key: "positive", label: "Positive recs" },
  ...
  stages.forEach((stage) => {
    [
      { label: "Observed", groups: baseline },
      { label: "Scenario", groups: scenarioGroups }
    ].forEach((mode) => {
      const men = d3.sum(mode.groups.filter((d) => !d.women), (d) => d[stage.key]);
      const women = d3.sum(mode.groups.filter((d) => d.women), (d) => d[stage.key]);
      rows.push({ stage: stage.label, mode: mode.label, men, women, total: men + women });
    });
  });

  const width = Math.max(container.node().clientWidth, 960);
  const height = 560;
  const margin = { top: 112, right: 126, bottom: 58, left: 202 };
  const x = d3.scaleLinear().domain([0, d3.max(rows, (d) => d.total)]).nice().range([margin.left, width - margin.right]);
  const y = d3.scaleBand().domain(rows.map((d) => `${d.stage}|${d.mode}`)).range([margin.top, height - margin.bottom]).paddingInner(0.24);
  const svg = container.append("svg").attr("width", "100%").attr("height", height).attr("viewBox", [0, 0, width, height]);

  const deltaAccepted = scenarioTotals.accepted - baselineTotals.accepted;
  const baselineWomenShare = baselineTotals.womenAccepted / baselineTotals.accepted;
  const scenarioWomenShare = scenarioTotals.womenAccepted / scenarioTotals.accepted;
  const metrics = [
    { label: "Accepted change", value: `${deltaAccepted >= 0 ? "+" : ""}${d3.format(",.0f")(deltaAccepted)}` },
    { label: "Women-involved accepted", value: `${pct(scenarioWomenShare)} vs ${pct(baselineWomenShare)}` },
    { label: "Scenario accepted", value: d3.format(",.0f")(scenarioTotals.accepted) }
  ];

  const metric = svg.append("g").attr("class", "sim-metrics").attr("transform", `translate(${margin.left},28)`);
  metric.selectAll("g")
    .data(metrics)
    .join("g")
    .attr("transform", (_, i) => `translate(${i * 196},0)`)
    .call((g) => {
      g.append("rect").attr("width", 178).attr("height", 56).attr("rx", 14);
      g.append("text").attr("class", "sim-metric-label").attr("x", 14).attr("y", 21).text((d) => d.label);
      g.append("text").attr("class", "sim-metric-value").attr("x", 14).attr("y", 43).text((d) => d.value);
    });

  svg.append("g")
    .attr("class", "grid sim-grid-lines")
    .attr("transform", `translate(0,${height - margin.bottom})`)
  ...
  const rowGroups = svg.append("g")
    ...
    .join("g")
    .attr("class", "sim-row")
    .attr("transform", (d) => `translate(0,${y(`${d.stage}|${d.mode}`)})`);

  rowGroups.append("text")
    .attr("class", "sim-stage-label")
    .attr("x", 24)
    .attr("y", y.bandwidth() / 2 + 4)
    .text((d) => d.mode === "Observed" ? d.stage : "");

  rowGroups.append("text")
    .attr("class", "sim-mode-label")
  ...
  rowGroups.append("rect")
    .attr("class", "sim-bar-men")
    .attr("x", margin.left)
    .attr("width", (d) => x(d.men) - margin.left)
    .attr("height", y.bandwidth())
    .attr("rx", 8);

  rowGroups.append("rect")
    .attr("class", "sim-bar-women")
    .attr("x", (d) => x(d.men))
    .attr("width", (d) => x(d.total) - x(d.men))
    .attr("height", y.bandwidth())
    .attr("rx", 8);

  rowGroups.append("text")
    .attr("class", "sim-total-label")
    .attr("x", (d) => Math.min(width - margin.right + 10, x(d.total) + 8))
    .attr("y", y.bandwidth() / 2 + 4)
    .text((d) => d3.format(",.0f")(d.total));

  const legend = svg.append("g").attr("class", "sim-legend").attr("transform", `translate(${width - margin.right - 280},${height - 24})`);
  [
    { label: "Men-only groups", color: "#6B8FB8" },
    { label: "Women-involved groups", color: "#C06C84" }
  ].forEach((d, i) => {
    const item = legend.append("g").attr("transform", `translate(${i * 146},0)`);
    item.append("rect").attr("width", 12).attr("height", 12).attr("rx", 3).attr("fill", d.color);
    item.append("text").attr("x", 18).attr("y", 10).text(d.label);
  });
}

function renderFairnessSimulator() {
  const base = buildSimulatorBase();
  if (!base) return;
  renderSimulatorPresets();
  renderSimulatorControls();
  renderSimulatorChart(base, applySimulatorScenario(base));
}
